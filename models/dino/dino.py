...
from semi import EMA, MixPL
from ..create_model import create_model
from models.feature_pyramid import calcFPN
...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        self.ema = EMA(self)
        self.mixpl = MixPL()
        self.FPN = calcFPN(in_channels=256)
        self.fasterrcnn = create_model(num_classes=num_classes + 1)
        self.fasterrcnn_ori = create_model(num_classes=num_classes + 1)
    ...
    def forward(self, samples: NestedTensor, targets:List=None, retmemory=False, onlymemory=False):
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal, reshaped_memory = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask, onlymemory)
        if onlymemory:
            return reshaped_memory, reshaped_memory
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        pb = 0
        if 'scores' in targets[0]:
            for t in targets:
                pb += len(t['boxes'])
            if pb != 0:
                loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction='none')
                bbox_weight = torch.cat(
                    [t["scores"][J] for t, (_, J) in zip(targets, indices) if len(t['boxes']) > 0]).unsqueeze(1)
                losses = {}
                if torch.sum(bbox_weight) < 1e-6:
                    losses['loss_bbox'] = 0
                    losses['loss_xy'] = 0
                    losses['loss_hw'] = 0
                    losses['loss_giou'] = 0
                    return losses
                losses['loss_bbox'] = (loss_bbox * bbox_weight).sum() / torch.sum(bbox_weight)
                with torch.no_grad():
                    losses['loss_xy'] = (loss_bbox * bbox_weight)[..., :2].sum() / torch.sum(bbox_weight)
                    losses['loss_hw'] = (loss_bbox * bbox_weight)[..., 2:].sum() / torch.sum(bbox_weight)
                loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(
                    box_ops.box_cxcywh_to_xyxy(src_boxes),
                    box_ops.box_cxcywh_to_xyxy(target_boxes)))
                bbox_weight = bbox_weight.view(-1)
                losses['loss_giou'] = (loss_giou * bbox_weight).sum() / torch.sum(bbox_weight)
                return losses
        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction='none')
