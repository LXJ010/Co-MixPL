...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, onlymemory=False):
        """
        Input:
        ...
        bs, _, channel = memory.shape
        reshaped_memory = []
        for i in range(len(level_start_index)):
            start_idx = level_start_index[i]
            end_idx = level_start_index[i + 1] if i + 1 < len(level_start_index) else memory.shape[1]
            memory_slice = memory[:, start_idx:end_idx, :].permute(0, 2, 1)
            hi, wi = spatial_shapes[i]
            reshaped_tensor = memory_slice.reshape(bs, channel, hi, wi)
            reshaped_memory.append(reshaped_tensor)
        if onlymemory:
            return reshaped_memory,reshaped_memory,reshaped_memory,reshaped_memory,reshaped_memory,reshaped_memory
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, reshaped_memory
        # hs: (n_dec, bs, nq, d_model)
